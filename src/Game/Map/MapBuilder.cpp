    ...
    for (int i = 0; i < mapImage.getWidth(); i++) {
        for (int j = 0; j < mapImage.getHeight(); j++) {
            ...
            if(currentPixel == boundBoundBlock){
                ...
                PacMan = new Player(xPos,yPos,pixelMultiplier,pixelMultiplier, entityManager /*, false */);
				Mario = new Player(xPos,yPos,pixelMultiplier,pixelMultiplier, entityManager /*, true */);
				mapInCreation->setPlayer(PacMan);
				mapInCreation->setAltPlayer(Mario);
				...
				if(ofRandom(0,20) >= 19.7) {
                CherryPowerUp* cherry = new CherryPowerUp(xPos,yPos,pixelMultiplier,pixelMultiplier, pacmanSpriteSheet);
                mapInCreation->addEntity(cherry);
                }
				else if (ofRandom(0,20)>= 19.7){
					StrawberryPowerUp* strawberry = new StrawberryPowerUp(xPos,yPos,pixelMultiplier,pixelMultiplier, pacmanSpriteSheet);
                mapInCreation->addEntity(strawberry);
				}
				else if (ofRandom(0,20)>= 19.7){
					ApplePowerUp* apple = new ApplePowerUp(xPos,yPos,pixelMultiplier,pixelMultiplier, pacmanSpriteSheet);
